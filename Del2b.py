...
temp_uis_max = int(np.ceil(max(temp_liste)))
temp_uis_min = int(np.floor(min(temp_liste)))

temp_sola_max = int(np.ceil(max(lufttemp_liste)))
temp_sola_min = int(np.floor(min(lufttemp_liste)))

bins_max = max(temp_uis_max, temp_sola_max)
bins_min = min(temp_uis_min, temp_sola_min)

bins_finale = np.arange(bins_min, bins_max + 1, 1)

plt.hist([temp_liste, lufttemp_liste], bins= bins_finale, label="Histogram av tempraturer")
plt.show()
